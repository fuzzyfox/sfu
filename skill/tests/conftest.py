...
# A stand-in `security` binary. It understands only the one shape the Token Store
# uses — `find-generic-password -s <service> -a <account> -w` — and answers from
# a JSON store the test populates. Absent items mirror the real CLI: nonzero exit
...
with open(os.environ["SFU_STUB_STORE"]) as fh:
    store = json.load(fh)
...
def security_stub(tmp_path, monkeypatch):
    ...
    return SecurityStub(store_path)
